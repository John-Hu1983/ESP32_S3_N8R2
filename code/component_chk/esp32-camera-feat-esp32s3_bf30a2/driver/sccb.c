...
int SCCB_Init(int pin_sda, int pin_scl)
{
    ...
    esp_err_t ret = i2c_driver_install(SCCB_I2C_PORT, conf.mode, 0, 0, 0);
    if (ret) {
        ESP_LOGE(TAG, "init iic fail");
    }
    ESP_LOGD(TAG, "init iic ok");
...
uint8_t SCCB_Probe(void)
{
    ...
    //     ESP_LOGD(TAG, "probe 0x%02X", i);
    //     i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    //     i2c_master_start(cmd);
    //     i2c_master_write_byte(cmd, ( i << 1 ) | WRITE_BIT, ACK_CHECK_EN);
    //     i2c_master_stop(cmd);
    //     esp_err_t ret = i2c_master_cmd_begin(SCCB_I2C_PORT, cmd, 100 / portTICK_RATE_MS);
    //     i2c_cmd_link_delete(cmd);
    //     if(ret == ESP_OK) {
    //         ESP_LOGW(TAG, "Found I2C Device at 0x%02X", i);
    //     } else {
    //         // ESP_LOGD(TAG, "probe 0x%02X fail", i);
    //     }
    // }
